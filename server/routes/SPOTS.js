...
router.get('/kakaoMap/search', (req, res) => {
	...
	const sqlSelect = 'SELECT S_NAME,S_ADDR,S_AMENITY FROM SPOTS WHERE S_NAME IS NOT NULL';
...
router.get('/kakaoMap', (req, res) => {
	...
	const sqlSelect = "SELECT * FROM SPOTS WHERE S_ADDR LIKE '전북%' OR S_ADDR LIKE '전남%' OR S_ADDR LIKE '광주%' ";
	db.query(sqlSelect, (err, result) => {
		res.json(result);
	});
});
...
router.post('/kakaoMap/local', (req, res) => {
	...
	switch (local) {
		case '전라도':
			sqlSelect = "SELECT * FROM SPOTS WHERE S_ADDR LIKE '%전남%' OR S_ADDR LIKE '%전북%' OR S_ADDR LIKE '%광주%'";
			break;
		case '강원도':
		...
		case '경기도':
			sqlSelect = "SELECT * FROM SPOTS WHERE S_ADDR LIKE '%경기%' OR S_ADDR LIKE '%서울%' OR S_ADDR LIKE '%인천%' ";
			break;
		...
		case '충청도':
			sqlSelect = "SELECT * FROM SPOTS WHERE S_ADDR LIKE '%충청%' OR S_ADDR LIKE '%세종%' OR S_ADDR LIKE '%대전%' ";
			break;
...
router.post('/kakaoMap/SPOT', (req, res) => {
	...
	switch (local) {
		case '전라도':
			...
			break;
		case '강원도':
			sqlSelect = "SELECT * FROM SPOTS WHERE (S_ADDR LIKE '%강원%')  AND S_CATEGORY= ?";
			break;
		...
		case '경상도':
			sqlSelect =
				"SELECT * FROM SPOTS WHERE (S_ADDR LIKE '%경상%' OR S_ADDR LIKE '%경남%' OR S_ADDR LIKE '%경북%' OR S_ADDR LIKE '%대구%' OR S_ADDR LIKE '%부산%' OR S_ADDR LIKE '%울산%') AND S_CATEGORY= ?  ";
			break;
		...
		case '제주도':
			sqlSelect = "SELECT * FROM SPOTS WHERE (S_ADDR LIKE '%제주%') AND S_CATEGORY= ? ";
			break;
	}
