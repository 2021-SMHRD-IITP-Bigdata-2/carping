...
async function predict() {
	...
	await axios.post('http://localhost:3001/SPOTS/predSpot', { predSpot: spotList[max] }).then((response) => {
...
const putSpots = () => {
	...
	recomends3.forEach((recomend, i) => {
		...
		if (filteredSpots == 0) {
			...
			recomend.querySelector('p').textContent = '(っ °Д °;)っ(っ °Д °;)っ(っ °Д °;)っ';
			...
			recomend.addEventListener('click', () => {
				modal.style.display = 'flex';
				document.querySelector('.spot-info-text>h1').innerText = filteredSpots[indexPutSpots[i]].S_NAME; // 이름
				document.querySelector('.spot-info-text > span').innerText = filteredSpots[indexPutSpots[i]].S_ADDR; // 주소
				document.querySelector('.spot-info-text > h3').innerText = filteredSpots[indexPutSpots[i]].S_PHONE; // 번호
				document.querySelector('.spot-img').src = filteredSpots[indexPutSpots[i]].S_IMG; // 이미지
				document.querySelectorAll('.spot-img')[1].src = filteredSpots[indexPutSpots[i]].S_IMG_D1; //작은 이미지
				document.querySelectorAll('.spot-img')[2].src = filteredSpots[indexPutSpots[i]].S_IMG_D2; //작은 이미지

				InnerModalImg.forEach((img, j) => {
					// 화장실 편의시설 표시해주는 기능
					if (filteredSpots[indexPutSpots[i]].S_AMENITY.includes(img.innerText)) {
						document.querySelector('.icon-option').children[j].style.display = 'flex';
						console.log(filteredSpots[indexPutSpots[i]].S_AMENITY);
					} else {
						document.querySelector('.icon-option').children[j].style.display = 'none';
					}
				});
			});
		}
...
const InitPutSpots = () => {
	...
	recomends3.forEach((recomend, i) => {
		...
		recomend.addEventListener('click', () => {
			modal.style.display = 'flex';
			document.querySelector('.spot-info-text>h1').innerText = spots[indexList[i]].S_NAME; // 이름
			document.querySelector('.spot-info-text > span').innerText = spots[indexList[i]].S_ADDR; // 주소
			document.querySelector('.spot-info-text > h3').innerText = spots[indexList[i]].S_PHONE; // 번호
			document.querySelector('.spot-img').src = spots[indexList[i]].S_IMG; // 이미지
			document.querySelectorAll('.spot-img')[1].src = spots[indexList[i]].S_IMG_D1; //작은 이미지
			document.querySelectorAll('.spot-img')[2].src = spots[indexList[i]].S_IMG_D2; //작은 이미지
			// document.querySelector(".spot-img").src = spots[indexList[i]].S_IMG; // 이미지
			// document.querySelector(".spot-img").src = spots[indexList[i]].S_IMG; // 이미지
