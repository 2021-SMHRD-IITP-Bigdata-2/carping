...
const spotFilter = () => {
  var currentLocal = document.getElementsByClassName("current")[0].innerText;
  const currentSpot = document.getElementsByClassName("current")[1].innerText;
  const list = document.getElementsByClassName("list")[1];
  const spotList = list.querySelectorAll("li");

  console.log(spotList);
  for (var i = 0; i < spotList.length; i++) {
    (function (spot) {
      spot.addEventListener("click", () => {
        currentLocal = document.getElementsByClassName("current")[0].innerText;
        console.log(spot.innerText);
        console.log(currentLocal);
        axios
          .post("http://localhost:3001/SPOTS/kakaoMap/SPOT", {
            local: currentLocal,
            spot: spot.innerText,
          })
          .then((response) => {
            ...
            var markers = [];
            const mapListwrap =
              document.getElementsByClassName("map-listwrap")[0];
    ...
    // console.log(currentLocal);
    // console.log(spotList[i].innerText);
  }
